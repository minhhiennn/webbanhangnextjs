...
export default function ResetPassword({ err, token }) {
    ...
    const checkSubmit = async () => {
        ...
        if (newPassword != confirmNewPassword) {
            ...
            const res = await axios.post('https://webbanhangsieuvip.herokuapp.com/api/accounts/reset-password', { token: tk, password: newPassword, confirmPassword: confirmNewPassword })
            if (res) {
...
export async function getServerSideProps({ query }) {
    let err1 = "";
    const res = await axios.post('https://webbanhangsieuvip.herokuapp.com/api/accounts/validate-reset-token', { token: query.token })
        .catch(err => {
