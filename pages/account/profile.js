...
export default function Profile({ keyword, customer }) {
    ...
    useEffect(() => {
        const changeInformationCustomer = async () => {
            const res = await instance().post("https://webbanhangsieuvip.herokuapp.com/api/customer/changeInformationCustomer", customerState)
                .catch(err => {
...
export async function getServerSideProps({ req, query }) {
    ...
    if (session) {
        const res = await instance({ req }).get("https://webbanhangsieuvip.herokuapp.com/api/customer/getCurrentCustomer")
