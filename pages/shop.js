...
export async function getServerSideProps(context) {
  try {
    ...
    const res = await InstanceAxios(context).get(`https://webbanhangsieuvip.herokuapp.com/api/product/loadAll?page=${page - 1}&size=${size}${catagory}${sorter}`)
    const res2 = await InstanceAxios(context).get(`https://webbanhangsieuvip.herokuapp.com/api/product/loadCagetory`)
