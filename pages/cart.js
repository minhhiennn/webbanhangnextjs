...
export default function Cart(props) {
  ...
  const changeQuantityCart = async (event, currentQuantity, product) => {
    ...
    if (target.value == "" || target.value == currentQuantity) {
      ...
      const res = await instance().post(`https://webbanhangsieuvip.herokuapp.com/api/cart/updateProduct`, { product, quantity: target.value })
        .catch((err) => {
          console.log(err);
      ...
      if (res) {
        const response = await instance().get("https://webbanhangsieuvip.herokuapp.com/api/cart/getCartByIdCustomer", { params: { idCustomer: props.user.id } });
        if (response) {
          removeDisableClick(product.idProduct);
          ...
          const responseShipServiceFee = await instance().post("https://webbanhangsieuvip.herokuapp.com/api/ghn/calculateFee", calFeeRequest)
  ...
  const addToCart = async (quantity, currentQuantity, product) => {
    ...
    disableClick(product.idProduct);
    const res = await instance().post(`https://webbanhangsieuvip.herokuapp.com/api/cart/addToCart`, { product, quantity: quantity })
      .catch((err) => {
        removeDisableClick(product.idProduct);
        if (err.message != "Network Error") {
          target.value = currentQuantity;
  ...
  const deletCart = async (quantity, product) => {
    const res = await instance().post(`https://webbanhangsieuvip.herokuapp.com/api/cart/deleteCartItem`, product).catch(() => { alert("không thể delete product") });
    if (res) {
      const response = await instance().get("https://webbanhangsieuvip.herokuapp.com/api/cart/getCartByIdCustomer", { params: { idCustomer: props.user.id } });
      if (response) {
        setCart(response.data);
  ...
  const addCartItemToOrder = async () => {
    ...
    const res1 = await instance().get(`https://webbanhangsieuvip.herokuapp.com/api/customer/checkPhoneExistCustomer`)
      .catch((err) => {
        alert(err.response.data.message);
      })
    if (res1) {
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      ...
      data.forEach(function (cartItem) { delete cartItem.product.imgUrl });
      const responseShipServiceType = await instance().post("https://webbanhangsieuvip.herokuapp.com/api/ghn/getServiceType", data)
...
export async function getServerSideProps(context) {
  ...
  if (session) {
    ...
    const responseCart = await axios.get("https://webbanhangsieuvip.herokuapp.com/api/cart/getCartByIdCustomer",
