...
export default function PhoneOTPProgress({ open, closeModal, shippingPriceList }) {
    ...
    const sendSMS = async () => {
        ...
        const res1 = await instance().get('https://webbanhangsieuvip.herokuapp.com/api/customer/getCurrentCustomer')
        if (res1) {
            const phoneNumber = res1.data.phoneNumber;
            const res = await instance().get('https://webbanhangsieuvip.herokuapp.com/api/customer/sendPhoneSMS')
            if (res) {
                const sms = res.data;
                const res2 = await instance().get('https://webbanhangsieuvip.herokuapp.com/api/sms/sendPhoneSMS', { params: { phoneNumber: phoneNumber, sms: sms } });
                if (res2) {
    ...
    const checkSMS = async () => {
        ...
        const res = await instance().get('https://webbanhangsieuvip.herokuapp.com/api/customer/onCompleteOrderPayment', { params: { sms: sms } })
            .catch(err => {
        ...
        if (res) {
            if (shippingPriceList != null) {
                ...
                const res1 = await instance().post(`https://webbanhangsieuvip.herokuapp.com/api/order/addCartItemToOrder`, req)
                    .catch((err) => {
                        alert(err.response.data.message)
