...
export default function ModalProfileEmail({ open, closeModal, customer, setCustomer, setFirstRender }) {
    ...
    const sendSMS = async () => {
        ...
        const res = await instance().get('https://webbanhangsieuvip.herokuapp.com/api/customer/sendGmailSMS', { params: { email: email } });
        if (res) {
    ...
    const checkSMS = async () => {
        ...
        const res = await instance().get('https://webbanhangsieuvip.herokuapp.com/api/customer/checkGmailSMS', { params: { sms: sms, email: email } })
            .catch(err => {
